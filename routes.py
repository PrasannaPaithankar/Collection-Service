...
from models import Agent, Customer, app, db, mail
...
def index():
    ...
    with open('/var/www/html/collection-service/report.csv', 'w') as f:
        f.write(
            "Name,Account Number,Mobile Number,Collection,Closing Balance,Agent ID\n")
    f.close()
...
def dashboard(agentID, name):
    if request.method == 'POST':
        ...
        if Customer.query.filter_by(id=int(acNo)-10000).first() is None:
            ...
            (Customer.query.filter_by(id=int(acNo)-10000).first()
             ).closingBalance += int(collection)
            (Customer.query.filter_by(id=int(acNo)-10000).first()
            ...
            with open('report.csv', 'a') as f:
                f.write(str((Customer.query.filter_by(id=int(acNo)-10000).first()).name)+","+str(acNo)+","+str((Customer.query.filter_by(id=int(acNo)-10000).first()
                                                                                                                ).mobileNo)+","+str(collection)+","+str((Customer.query.filter_by(id=int(acNo)-10000).first()).closingBalance)+","+str(agentID)+"\n")
            f.close()
...
def add():
    if request.method == 'POST':
        ...
        if Agent.query.filter_by(email=email).first() is None:
            agent = Agent(name=name, mobileNo=mobileNo,
                          email=email, password=password)
            db.session.add(agent)
            db.session.commit()
            message = "Your account has been created. Your agent ID is " + \
                str(10000+agent.id) + " and password is " + password
            # send message to agent
            webbrowser.open('https://api.whatsapp.com/send?phone=91' +
                            str(mobileNo)+'&text='+message, new=2, autoraise=False)
            time.sleep(1)
            pyautogui.hotkey('enter')

            mail.send_message('User authentication', sender='vrslightmodecoders@gmail.com',
                              recipients=[str(agent.email)], body=message)
...
def addCustomer(agentID):
    if request.method == 'POST':
        ...
        if Customer.query.filter_by(mobileNo=int(mobileNo)).first() is None and password == (Agent.query.filter_by(id=int(agentID)-10000).first()).password:
            customer = Customer(
                name=name, mobileNo=mobileNo, agentID=int(agentID))
            db.session.add(customer)
            db.session.commit()
            message = "Your account has been created. Your account number is " + \
                str(10000+customer.id)
            # send message to customer
            webbrowser.open('https://api.whatsapp.com/send?phone=91' +
                            str(mobileNo)+'&text='+message, new=2, autoraise=False)
            time.sleep(1)
    ...
    if request.method == 'POST':
        ...
        if Customer.query.filter_by(id=int(search)-10000).first() is not None or Customer.query.filter_by(name=search).first() is not None or Customer.query.filter_by(mobileNo=int(search)).first() is not None:
            if search.isnumeric():
                ...
                acNo = 10000 + \
                    (Customer.query.filter_by(mobileNo=int(search)).first()).id
    ...
    if request.method == 'POST':
        ...
        if Agent.query.filter_by(id=int(search)-10000).first() is not None or Agent.query.filter_by(name=search).first() is not None or Agent.query.filter_by(email=search).first() is not None:
            if search.isnumeric():
                ...
                agentID = 10000 + \
                    (Agent.query.filter_by(email=search).first()).id
...
def admin_action():
    if request.method == 'POST':
        ...
        if "Export" in request.form.getlist('option'):
            shutil.copy('/var/www/html/collection-service/report.csv',
                        '/var/www/html/collection-service/Reports/report-{}.csv'.format(str(datetime.datetime.now())[:10]))
            with open('/var/www/html/collection-service/report.csv', 'w') as f:
                f.write(
                    "Name,Account Number,Mobile Number,Collection,Closing Balance,Agent ID\n")
            f.close()
        ...
        if "Add Agents" in request.form.getlist('option'):
            ...
            for i in range(1, len(df)):
                agent = Agent(name=df['Name'][i], email=df['Email'][i], password=df['Password']
                              [i], mobileNo=df['Mobile Number'][i], status=df['Status'][i])
                db.session.add(agent)
                db.session.commit()
                message = "Your account has been created. Your agent ID is " + \
                    str(10000+agent.id) + " and password is " + agent.password
                # send message to agent
                webbrowser.open('https://api.whatsapp.com/send?phone=91' +
                                str(agent.mobileNo)+'&text='+message, new=2, autoraise=False)
                time.sleep(1)
                pyautogui.hotkey('enter')
        ...
        if "Add Customers" in request.form.getlist('option'):
            ...
            for i in range(1, len(df)):
                customer = Customer(
                    name=df['Name'][i], mobileNo=df['Mobile Number'][i], agentID=df['Agent ID'][i])
                db.session.add(customer)
                db.session.commit()
                message = "Your account has been created. Your account number is " + \
                    str(10000+customer.id)
                # send message to customer
                webbrowser.open('https://api.whatsapp.com/send?phone=91' +
                                str(customer.mobileNo)+'&text='+message, new=2, autoraise=False)
                time.sleep(1)
                pyautogui.hotkey('enter')
...
def message():
    if request.method == 'POST':
        ...
        if request.form.get('toall') == "a":
            ...
            emails = [i.email for i in Agent.query.filter(
                Agent.name.in_(temp)).all()]
        print(message)
        mail.send_message(
            'User authentication', sender='vrslightmodecoders@gmail.com', recipients=emails, body=message)
...
def blockAgent(agentID):
    Agent.query.filter_by(id=int(agentID)-10000).update(dict(status=1 -
                                                             int((Agent.query.filter_by(id=int(agentID)-10000).first()).status)))
    db.session.commit()
    return redirect(url_for("view", agentID=agentID))
...
def chpass(agentID):
    if request.method == 'POST':
        ...
        Agent.query.filter_by(
            id=int(agentID)-10000).update(dict(password=password))
        db.session.commit()
        return redirect(url_for("view", agentID=agentID))
    return redirect(url_for("view", agentID=agentID))
